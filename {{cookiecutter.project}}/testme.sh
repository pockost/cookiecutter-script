...
function ansible_tests_exists() {
    ...
    ls $test_path/*.yml &>/dev/null
    [ $? -eq 0 ] && echo true && return 0

    # Search for .yaml tests.
    ls $test_path/*.yaml &>/dev/null
    [ $? -eq 0 ] && echo true && return 0
...
function coverage_report() {
    ...
    local test_dir=$(get_test_dir $project_path)

    # Check if bare metal tests exists.
    ls $test_dir/*.py &>/dev/null
    [ $? -eq 1 ] && return 1
...
function python_tests_exists() {
    ...
    # Bare metal tests.
    # Search the playbook $test_dir/test-playbook.yml.
    ls $test_dir/test-playbook.yml &>/dev/null
    [ $? -eq 0 ] && echo true && return 0
...
function setup_ansible_python_modules() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if ! [[ -d $test_dir/library ]]; then
            ln -s $project_path/library $test_dir/library
        fi
...
function setup_ansible_python_plugins() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if ! [[ -d $test_dir/test_plugins ]]; then
            ln -fs $project_path/test_plugins $test_dir/test_plugins
        fi
...
function tests_ansible() {
    ...
    ls $test_dir/*.yml &>/dev/null
    [ $? -eq 0 ] && yml_files=$(ls $test_dir/*.yml)

    ls $test_dir/*.yaml &>/dev/null
    [ $? -eq 0 ] && yml_files="$yml_files $(ls $test_dir/*.yaml)"
    ...
    for playbook in $yml_files; do
        ...
        if ! [[ -f $test_dir/inventory ]]; then
            echo 'localhost' > $test_dir/inventory
        fi

        ansible-playbook -i $test_dir/inventory $playbook -e \
...
function tests_bats() {
    ...
    ls $test_dir/*.bats &>/dev/null
    [ $? -eq 1 ] && return 0

    [[ $(validate 'bats') == false ]] && return 1
    [[ -d $test_dir ]] && bats $test_dir
